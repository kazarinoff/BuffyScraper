...
public class buffyparser {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i < 8; i++) {
            String buffyurl = String.format("https://en.wikipedia.org/wiki/Buffy_the_Vampire_Slayer_(season_%s)",
                    Integer.toString(i));
            try {
                ...
                while (true) {
                    ...
                    node = node.nextElementSibling();
                    if (node.text().equals("N/A")) {
                        episode.put("viewwership", node.text());
                    } else {
                    ...
                    try {
                        node = node.nextElementSibling().child(0);
